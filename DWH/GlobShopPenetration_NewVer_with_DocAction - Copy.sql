...
select [fDC].[date_ID] datCheck, COUNT(tovar_ID) kvo into #CustomCountChecks
...
--select [fDC].[date_ID], count(distinct [fDC].[check_num]) count_all_checks, count(fDC1.tovar_ID) from [wh].[dbo].[fact_DocCash] fDC join [wh].[dbo].[fact_DocCash] fDC1
--ON [fDC].[date_ID] = [fDC1].[date_ID] 
--where [fDC].[date_ID] Between @startDate AND @endDate and [fDC].[oper_type_ID] in (150401,150404) and [fDC1].[tovar_ID] in (select distinct tovar_id from cba.dbo.mtm_fb_prior where doc_id = 3102)
--group by fDC.date_ID HAVING count(fDC1.tovar_ID) = 1 order by fDC.date_ID
