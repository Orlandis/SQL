...
select [fDC].[date_ID] dateBuy, [fDC].[node_ID] node, [dn].[name] nameShop, [fDC].[kassa_ID] kasID, count(distinct check_num) count_position into #GlobCountChecks
from [wh].[dbo].[fact_DocCash] fDC join [dbo].[dim_node] dn on [fDC].[node_ID] = [dn].[node_ID] 
where [fDC].[date_ID] Between @startDate AND @endDate and [oper_type_ID] in (150401,150404) and fDC.node_ID in (299)
...
select distinct [fDC].[check_num] checkN, [fDC].[date_ID] dateBuy, [fDC].[node_ID] node, [dn].[name] nameShop, [fDC].[kassa_ID] kasID, COUNT(tovar_ID) count_position into #CustomCountChecks
from [wh].[dbo].[fact_DocCash] fDC join [dbo].[dim_node] dn on [fDC].[node_ID] = [dn].[node_ID] 
where [fDC].[date_ID] Between @startDate AND @endDate and [oper_type_ID] in (150401,150404) and fDC.node_ID in (299)
...
onePosition, morePosition as 'MoreThen >2', ((onePosition + morePosition) / (cast(count_position as float)) * 100)
